...
export const directusApi = {
  ...
  // Función auxiliar para filtrar gastos por período
  filterGastosPorPeriodo(gastosArray: any[], dias: number) {
    const hoy = new Date();
    const fechaLimite = new Date();
    fechaLimite.setDate(hoy.getDate() + dias);

    return gastosArray.filter((gasto: any) => {
      // Solo gastos pendientes o vencidos
      if (gasto.EstadoPago !== 'Pendiente' && gasto.EstadoPago !== 'Vencido') {
        return false;
      }

      // Si no tiene fecha de vencimiento, se incluye (se considera que vence pronto)
      if (!gasto.ChequeFechaVencimiento) {
        return true;
      }

      // Si tiene fecha, verificar que esté dentro del período
      const fechaVencimiento = new Date(gasto.ChequeFechaVencimiento);
  ...
  // NUEVAS FUNCIONES PARA PERÍODOS ESPECÍFICOS
  
  // Estadísticas para 30 días
  async getDashboardStats30() {
    try {
      const [clientes, proyectos, gastos, ingresos] = await Promise.all([
        this.getClientes(),
        this.getProyectos(),
      ...
      // Filtrar gastos por 30 días
      const gastos30Dias = this.filterGastosPorPeriodo(gastosArray, 30);
      const porPagar30 = gastos30Dias.reduce((sum: number, g: any) => sum + (g.Importe || 0), 0);

      // Por cobrar se mantiene igual (sin filtro de tiempo)
      const porCobrar = proyectosArray.reduce((sum: number, proyecto: any) => {
        const totalProyecto = proyecto.Importe || 0;
      ...
      const flujoNeto30 = porCobrar - porPagar30;

      return {
        porCobrar: porCobrar,
        porPagar: porPagar30,
        flujoNeto: flujoNeto30
      };
    } catch (error) {
      console.error('Error calculando stats 30 días:', error);
      return {
        porCobrar: 0,
        porPagar: 0,
        flujoNeto: 0
  ...
  async getDashboardStats60() {
    try {
      const [proyectos, gastos, ingresos] = await Promise.all([
        this.getProyectos(),
        this.getGastos(),
        this.getIngresos()
      ]);
      ...
      // Filtrar gastos por 60 días
      const gastos60Dias = this.filterGastosPorPeriodo(gastosArray, 60);
      const porPagar60 = gastos60Dias.reduce((sum: number, g: any) => sum + (g.Importe || 0), 0);
      ...
      const flujoNeto60 = porCobrar - porPagar60;

      return {
        porCobrar: porCobrar,
        porPagar: porPagar60,
        flujoNeto: flujoNeto60
      };
    } catch (error) {
      console.error('Error calculando stats 60 días:', error);
      return {
        porCobrar: 0,
        porPagar: 0,
        flujoNeto: 0
  ...
  // CRUD Operations - SE MANTIENEN TODOS IGUALES
...
async createGasto(gasto: any) {
  try {
    ...
    const cleanedGasto = {
      ...gasto,
      ChequeFechaVencimiento: gasto.ChequeFechaVencimiento === "" || gasto.ChequeFechaVencimiento === null 
        ? null 
        : gasto.ChequeFechaVencimiento,
      FechaPago: gasto.FechaPago === "" || gasto.FechaPago === null 
        ? null 
        : gasto.FechaPago
    };
    ...
    const response = await fetch(`${API_BASE}/items/RaizGastos`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(cleanedGasto), // ← Ahora usa cleanedGasto
    });
    const data = await response.json();
...
async updateGasto(id: number, gasto: any) {
  try {
    ...
    const response = await fetch(`${API_BASE}/items/RaizGastos/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(cleanedGasto),
    });
    const data = await response.json();
