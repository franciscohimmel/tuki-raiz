...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  // Estados para los períodos específicos
  const [stats30, setStats30] = useState({ porCobrar: 0, porPagar: 0, flujoNeto: 0 });
  const [stats60, setStats60] = useState({ porCobrar: 0, porPagar: 0, flujoNeto: 0 });
  const [loading30, setLoading30] = useState(true);
  const [loading60, setLoading60] = useState(true);
  ...
  // Cargar estadísticas por períodos
  useEffect(() => {
    const fetchStats30 = async () => {
      try {
        setLoading30(true);
        const data = await directusApi.getDashboardStats30();
        setStats30(data);
      } catch (error) {
        console.error('Error cargando stats 30:', error);
      } finally {
        setLoading30(false);
      }
    };

    const fetchStats60 = async () => {
      try {
        setLoading60(true);
        const data = await directusApi.getDashboardStats60();
        setStats60(data);
      } catch (error) {
        console.error('Error cargando stats 60:', error);
      } finally {
        setLoading60(false);
      }
    };
  ...
  // Filtrar gastos para 60 días en la sección "Por Pagar"
  const getGastosPorPagar60Dias = () => {
    const hoy = new Date();
    ...
    return gastos.filter(gasto => {
      ...
      // Si no tiene fecha de vencimiento, se incluye
      if (!gasto.ChequeFechaVencimiento) {
        return true;
      }

      // Si tiene fecha, verificar que esté dentro de 60 días
          ...
          <div className="dashboard-container">
            ...
            <div className="saldo-consolidado">
              ...
              <div className="saldo-card saldo-card-disponible">
                ...
                <div className="saldo-card-value">
                  {loading30 ? '...' : formatCurrency(stats30.flujoNeto)}
                </div>
              ...
              <div className="saldo-card saldo-card-pendiente-cobrar">
                ...
                <div className="saldo-card-value">
                  {loading30 ? '...' : formatCurrency(stats30.porCobrar)}
                </div>
              ...
              <div className="saldo-card saldo-card-pendiente-pagar">
                ...
                <div className="saldo-card-value">
                  {loading30 ? '...' : formatCurrency(stats30.porPagar)}
                </div>
            ...
            <div className="calendario-grid">
              ...
              <div className="calendario-box">
                <div className="calendario-header">
                  ...
                  <div className="calendario-total total-cobrar">
                    {loading60 ? '...' : formatCurrency(stats60.porCobrar)}
                  </div>
                </div>
              ...
              <div className="calendario-box">
                <div className="calendario-header">
                  ...
                  <div className="calendario-total total-pagar">
                    {loading60 ? '...' : formatCurrency(stats60.porPagar)}
                  </div>
                </div>
                <div className="calendario-content">
                  <div className="fecha-grupo">
                    ...
                    {(() => {
                      const gastosPorPagar = getGastosPorPagar60Dias();
                      return gastosPorPagar.length === 0 ? (
                        <div className="no-data">No hay gastos pendientes</div>
                      ) : (
                        gastosPorPagar.slice(0, 5).map((gasto) => (
                            ...
                            <div className="concepto-descripcion">
                              ...
                              <div className="concepto-proyecto">
                                {gasto.ChequeFechaVencimiento 
                                  ? `Vence: ${new Date(gasto.ChequeFechaVencimiento).toLocaleDateString()}`
